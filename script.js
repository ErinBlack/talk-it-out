...
/*
2.) Create conditional logic to check if 'majorKey' is greater then or equal to 53
...
6. Create a new variable. Loop **backwards** through `keyChars` and store each value into the new variable, combining each of the values of `keyChars`*/
  ...
  for(i=keyChars.length-1; i>=0; i-- ){
    var backKey = keyChars[i];
    backwardsChars += backKey;
  }
...
*/
  ...
  backwardsChars = parseInt(backwardsChars);
...
*/
majorKey = majorKey += backwardsChars;
...
function keyVal (val) {
  ...
  if (newString.length > 1) {
    var newSlice = newString.substring(1);
  } else{ return false;}
  majorKey = newSlice;
  return majorKey;
}
